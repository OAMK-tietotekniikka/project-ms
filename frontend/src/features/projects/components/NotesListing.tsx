...
import {FileText, Trash2, Link, MessageSquare, Target, Plus, Search, Milestone} from "lucide-react";
...
import not_found_3 from '@/assets/not-found-3.svg';
...
const getItemHeight = (note) => {
	...
	const content = note.note_type === 'link'
		? note.note_url
...
const QuickAdd = ({ onAdd, isLoading }) => {
	...
	const submit = async (e) => {
		...
		const data = type === 'link'
			? { note_type: 'link', note_url: value, note_title: '', note_content: '' }
			: { note_type: type, note_content: value, note_title: '', note_url: '' };
...
const NotesListing = ({ project }) => {
	...
	const addNote = async (noteData) => {
		await addNoteMutation.mutateAsync({
			projectId: project.project_id,
			data: { ...noteData, created_by_name: localStorage.getItem("role") || "" }
		});
	};
		...
		if (searchTerm.trim()) {
			...
			filtered = filtered.filter(note => {
				...
				return content.toLowerCase().includes(search) ||
					(note.created_by_name && note.created_by_name.toLowerCase().includes(search));
			});
		}
		...
		<div className="space-y-4">
			...
			<div className="flex gap-3">
				...
				<Select value={filterBy} onValueChange={setFilterBy}>
					...
					<SelectContent>
						<SelectItem value="all">{t('projects_notesCategoryAll')}</SelectItem>
