...
import {ResultSetHeader, RowDataPacket, FieldPacket, PoolConnection} from "mysql2/promise";
...
function getChangedFields(
    ...
    const changedFields: Array<{ key: string; value: string | null | undefined }> = [];
...
export const getStudents = async (
    ...
    try {
        ...
        const [students] = await connection.query(QUERY.SELECT_STUDENTS) as [RowDataPacket[], FieldPacket[]];
...
export const getStudent = async (
    ...
    try {
        connection = await pool.getConnection();
        const [student] = await connection.query<RowDataPacket[]>(QUERY.SELECT_STUDENT_BY_EMAIL, [email]);
        if (student.length > 0) {
...
export const createStudent = async (req: Request, res: Response): Promise<Response<HttpResponse>> => {
    ...
    try {
            ...
            const [q] = await connection.query<ResultSetHeader>(QUERY.CREATE_STUDENT,
                [student_name, email, class_code || null]);
            const id = q.insertId;
            const [new_student] = await connection.query<RowDataPacket[]>(QUERY.SELECT_STUDENT, id)
...
export const updateStudent = async (
    ...
    try {
        ...
        const [student] = await connection.query<RowDataPacket[]>(QUERY.SELECT_STUDENT, [req.params.student_id]);
        if (student.length === 0) {
            return responseHelper.notFound(res);
        }
        ...
        const queryValues = [...fieldsToUpdate.map(field => field.value), req.params.student_id];
